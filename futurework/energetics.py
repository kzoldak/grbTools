...
class Energetics(object):
    ...
    @property    
    def model_dict_example(self):
        """
        Doing this will set up your dict in the proper form:
          energetics = Energetics()
          paramDict = eval(energetics.model_dict_example)
        then you can update the parameter values using:
          paramDict['band']['alpha'] = -1.23
          etc...

        """
        if self.modelname == 'band':
            return ("{'band':{'alpha':-1.0, 'beta':-2.5, 'enterm':500.0, "
                    "'norm':0.01, 'entype':'epeak'}}")
        elif self.modelname == 'sbpl':
            return ("{'band':{'alpha':-1.0, 'beta':-2.5, 'enterm':500.0, "
                    "'norm':0.01, 'entype':'ebreak'}}")
        else:
            pass
